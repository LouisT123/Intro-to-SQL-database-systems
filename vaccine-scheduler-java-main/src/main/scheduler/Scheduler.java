...
public class Scheduler {
    ...
    private static void reserve(String[] tokens) {
        ...
        try {
            ...
            //check if caregiver free
            String s = " SELECT Username FROM Availabilities WHERE Time = ?; ";
            PreparedStatement statement = con.prepareStatement(s);
            statement.setDate(1, d);
            ResultSet resultSet = statement.executeQuery();
            if (!resultSet.isBeforeFirst()) {
                System.out.println("Caregiver not available on " + date);
                return;
            }
            resultSet.next();
            ...
            if (vaccine == null){
                ...
                try {
                    ...
                    Appointment appointment = new Appointment.AppointmentBuilder
                            (appointment_id,vaccineName, d, caregiver, currentPatient.getUsername()).build();
